...
function authenticateToken(request, response, next) {
    ...
    if(jwtToken === undefined){
        ...
        jwt.verify(jwtToken, "MT_SECRET_TOKEN", async (error, payload) => {
            if(error){
                response.status(401)
                response.send("Invalid JWT Token")
            } else{
...
app.get("/states/", authenticateToken, async (request, response) => {
    ...
    response.send(statesArray.map((eachState) => convertStateDbObjectToResponseObject(eachState)));
})
...
app.put("/districts/districtId/", authenticateToken, async(request, response) => {
    ...
    const updateDistrictQuery = `update set district_name = '${districtName}', state_id = ${stateId}, cases = '${cases}', cured = '${cured}', active = '${active}', deaths = '${deaths}'
